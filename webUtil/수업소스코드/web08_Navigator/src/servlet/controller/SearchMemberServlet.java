...
public class SearchMemberServlet extends HttpServlet {
	 ...
	 * 2. MemberDAO 리턴받고
	 * 3. 비지니스 로직 호출 !!!!
	 * 4). 3번의 결과값을 Attribute에 바인딩 --- 회원가입...void
	 * 5. 네비게이션
	 */
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("forwarding...일떄는 이곳으로 응답이 돌아오지 않습니다...");
	}
}
/*
 ...
 제어권이 완전이 이동하는 jsp 페이지로 가버리기 때문에 다시 서블릿으로 되돌아오지 않는다.
 그렇게 때문에 서블릿에서 PrintWriter 객체를 사용할 일이 전혀 없다...응답이 이곳에서 이뤄지지 않는다는 의미이다.
 
 응답의 내용을 jsp이지만
 출력된 결과창의 주소는 확인하면 전권을 가지고 이동했기 때문에 forwarding이 일어난 서블릿 주소가 뜨는 것을 확인할수 있다.
 -------------------------------------------------------------------------------------------
 2. include 방식
 서블릿의 PrintWriter객체를 이용하려 한다면 include방식으로 페이지를 이동해야 한다.
 include에 연결되 페이지의 모든 내용을 가지고 다시 서블릿으로 돌아오기 때문에 include한 다음에 있는 출력내용을 모두다
 응답을 한다.
 ...
 ServletRequest 객체 바인딩에 대해서 잘 다뤄야 한다.
 */
