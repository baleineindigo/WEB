...
public class BookDAOImpl implements BookDAO{
	...
	@Override
	public ArrayList<Book> showAllBooks(String category, String categoryContents) throws SQLException {
		Connection conn = null;
		PreparedStatement ps = null;	
		ResultSet rs = null;
		Book book = null;
		...
		String query = "";
		int categoryPrice= 0;
		String queryAll = "select isbn, title, catalogue, nation, publish_date, publisher, author, price, currency, description from book";
		String queryTitle = "select isbn, title, catalogue, nation, publish_date, publisher, author, price, currency, description from book where title LIKE ?";
		String queryPublisher = "select isbn, title, catalogue, nation, publish_date, publisher, author, price, currency, description from book where publisher LIKE ?";
		String queryPrice = "select isbn, title, catalogue, nation, publish_date, publisher, author, price, currency, description from book where price <= ?";
		
		try {
			conn = getConnection();
			if(category.equals("전체")) query = queryAll;
			else if(category.equals("도서명")) query = queryTitle;
			else if(category.equals("출판사")) query = queryPublisher;
			else if(category.equals("가격")) {query = queryPrice; categoryPrice = Integer.parseInt(categoryContents);}
			
			
			ps = conn.prepareStatement(query);
			if(query.equals(queryPrice)) ps.setInt(1, categoryPrice); 
			else if(query.equals(queryTitle) || query.equals(queryPublisher)) ps.setString(1, "%"+categoryContents+"%");
