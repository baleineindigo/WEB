...
public class MemberDAOImpl implements MemberDAO{
	...
	public void registerMember(MemberVO vo) throws SQLException {		
		...
		try{
			conn=  getConnection();
			String query = "INSERT INTO cafemember VALUES(?,?,?,?)";
	...
	public MemberVO findByIdMember(String id) throws SQLException {
		...
		try{
			conn = getConnection();
			String query = "SELECT * FROM cafemember WHERE id=?";
			ps = conn.prepareStatement(query);
	...
	public MemberVO login(String id, String password) throws SQLException{
		...
		try {
			conn = getConnection();
			String query = "SELECT * FROM cafemember WHERE id=? AND password=?";
			ps = conn.prepareStatement(query);
			ps.setString(1, id);
	...
	public boolean updateMember(MemberVO vo) throws SQLException{
		...
		try {
			conn= getConnection();
			String query = "UPDATE cafemember SET password=?, name=?, address=? WHERE id=?";
			ps = conn.prepareStatement(query);
	...
	public boolean idExist(String id) throws SQLException{
		...
		try {
			conn = getConnection();
			String query = "SELECT id FROM cafemember WHERE id=?";
			ps = conn.prepareStatement(query);
			ps.setString(1, id);
			rs = ps.executeQuery();
			return rs.next(); 
